    ...
    <div className={`space-y-6 ${className}`}>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        ...
        {topCities.length > 0 ? (
          ...
          <p className="text-gray-500 text-sm">Nenhuma assinatura com cidade informada</p>
        )}
      </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="text-lg font-semibold text-gray-900">Top 5 Estados</h3>
        </div>
        ...
        {topStates.length > 0 ? (
          ...
          <p className="text-gray-500 text-sm">Nenhuma assinatura com estado informado</p>
        )}
      </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="text-lg font-semibold text-gray-900">Assinaturas por Mês</h3>
        </div>
        ...
        {monthlyData.length > 0 ? (
          ...
          <p className="text-gray-500 text-sm">Nenhuma assinatura cadastrada</p>
        )}
      </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="text-center p-4 bg-blue-50 rounded-lg">
            ...
            <div className="text-sm text-blue-800">Total de Assinaturas</div>
          </div>
          <div className="text-center p-4 bg-green-50 rounded-lg">
            <div className="text-2xl font-bold text-green-600">{Object.keys(cityStats).length}</div>
            <div className="text-sm text-green-800">Cidades Diferentes</div>
          </div>
          <div className="text-center p-4 bg-purple-50 rounded-lg">
            <div className="text-2xl font-bold text-purple-600">{Object.keys(stateStats).length}</div>
            <div className="text-sm text-purple-800">Estados Diferentes</div>
          </div>
          <div className="text-center p-4 bg-orange-50 rounded-lg">
            <div className="text-2xl font-bold text-orange-600">
              {signatures.length > 0 ? Math.round(signatures.length / Object.keys(monthlyStats).length) : 0}
            </div>
            <div className="text-sm text-orange-800">Média por Mês</div>
          </div>
        </div>
      </div>
    </div>
