...
console.log('🔍 ANÁLISE DO SUPABASE ONLINE');
console.log('============================');

async function analisarSupabaseOnline() {
  try {
    console.log('📋 ETAPA 1: Verificando conexão...');
    ...
    console.log('\n📋 ETAPA 2: Verificando estrutura das tabelas...');
    ...
    console.log('\n📋 ETAPA 4: Verificando políticas RLS...');
    ...
    console.log('\n📋 ETAPA 5: Verificando índices...');
    
    // Verificar índices
    try {
      const { data: indexData, error: indexError } = await onlineSupabase
        .from('pg_indexes')
        .select('*')
        .like('tablename', 'kanban_%');
    ...
    console.log(`   - Tabelas: ${tabelas.length} verificadas`);
...
// Executar análise
analisarSupabaseOnline().catch(console.error);
